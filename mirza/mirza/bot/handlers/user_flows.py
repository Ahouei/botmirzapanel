...
async def free_trial(cb: CallbackQuery, session, user):
    limit = (await session.execute(select(BotSetting.value).where(BotSetting.key == "limit_usertest_all"))).scalar_one_or_none()
    limit = int(limit or 1)
    used = user.test_limit or 0
    if used >= limit:
        return await cb.message.answer(t("users.test.no_more"))
    panel_row = (await session.execute(select(PanelServer).where(PanelServer.allow_test.is_(True), PanelServer.enabled.is_(True)))).scalars().first()
    if panel_row is None:
        return await cb.message.answer("⚠️ no test panels configured")
    vol = int((await session.execute(select(BotSetting.value).where(BotSetting.key == "val_usertest"))).scalar_one_or_none() or 1)
    days = int((await session.execute(select(BotSetting.value).where(BotSetting.key == "time_usertest"))).scalar_one_or_none() or 1)
    try:
        res = await PurchaseService(session).provision(user.id, custom={"location": panel_row.name, "volume_gb": vol, "duration_days": days, "price": 0, "name": "usertest"}, is_test=True)
    ...
    extra = ""
    if res.panel_user.links:
        extra = "\n\n" + "\n".join(res.panel_user.links[:2])
    await cb.message.answer(t("users.test.ok", sub_url=sub or "-") + extra, reply_markup=kb)
...
async def service_detail(cb: CallbackQuery, session):
    ...
    # try to fetch live usage from panel
    used_str = "-"
    total_str = f"{inv.volume_gb or '∞'} GB"
    expires = "?"
    ...
    try:
        ...
        row = (await session.execute(select(PanelServer).where(PanelServer.name == inv.panel_name))).scalar_one_or_none()
        if row:
            ...
            if cls:
                ...
                pu = await api.get_user(inv.service_username or "")
                if pu:
                    used_str = _fmt_bytes(pu.used_bytes)
                    if pu.total_bytes:
                        total_str = _fmt_bytes(pu.total_bytes)
                    if pu.expires_at:
                        expires = pu.expires_at.strftime("%Y-%m-%d %H:%M")
    except Exception as e:
        log.warning("service_detail.live_fetch_failed", err=str(e))
    cfg = inv.config_payload or {}
    text = t("users.config.show", username=inv.service_username or "-", used=used_str, total=total_str, expires=expires)
    kb_rows = []
    if cfg.get("subscription_url"):
        sub = cfg["subscription_url"]
        if sub.startswith("http"):
            kb_rows.append([InlineKeyboardButton(text="🔗 Subscription", url=sub)])
        else:
            kb_rows.append([InlineKeyboardButton(text="📋 Copy link", callback_data=f"copy:{inv.id}")])
    if cfg.get("links"):
        # inline links as buttons if short
        for link in (cfg["links"][:3]):
            if isinstance(link, str) and link.startswith("http"):
                kb_rows.append([InlineKeyboardButton(text="🔗 Config link", url=link)])
    kb_rows.append([InlineKeyboardButton(text="♻️ Renew", callback_data=f"renew:{inv.id}"), InlineKeyboardButton(text="📊 Extra volume", callback_data=f"extra:{inv.id}")])
...
async def copy_link(cb: CallbackQuery, session):
    inv_id = cb.data.split(":", 1)[1]
    inv = await session.get(Invoice, inv_id)
    sub = (inv.config_payload or {}).get("subscription_url") if inv else None
    await cb.message.answer(f"`{sub or '-'}```", parse_mode="Markdown")
    await cb.answer()
...
async def send_qr(cb: CallbackQuery, session):
    ...
    if not sub:
        return await cb.answer(t("users.config.not_found"), show_alert=True)
    # also try links[0]
    if not sub.startswith("http") and (inv.config_payload or {}).get("links"):
        sub = (inv.config_payload.get("links") or [None])[0] or sub
    try:
        import qrcode
        ...
        buf.seek(0)
        await cb.message.answer_photo(BufferedInputFile(buf.getvalue(), filename="sub.png"), caption=t("users.config.qr_caption"))
    except ImportError:
        await cb.message.answer(f"`{sub}`", parse_mode="Markdown")
    except Exception as e:
        await cb.message.answer(f"`{sub}`\nQR error: {e}", parse_mode="Markdown")
    await cb.answer()
...
async def renew_menu(cb: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{d}d", callback_data=f"ren_days:{d}")] for d in (7, 14, 30, 60, 90)])
    await cb.message.answer(t("users.renew.ask_days"), reply_markup=kb)
    await cb.answer()


@router.callback_query(F.data.startswith("ren_days:"))
async def do_renew(cb: CallbackQuery, session, user, state: FSMContext):
    days = int(cb.data.split(":", 1)[1])
    data = await state.get_data()
    inv_id = data.get("renew_inv")
    if not inv_id:
        # fallback: most recent invoice
        inv = ((await session.execute(select(Invoice).where(Invoice.user_id == user.id).order_by(Invoice.sold_at.desc()))).scalars().first())
        if not inv:
            await cb.answer("No invoice", show_alert=True)
            return
        inv_id = inv.id
    per_day = int((await session.execute(select(BotSetting.value).where(BotSetting.key == "price_per_day"))).scalar_one_or_none() or 1000)
    cost = days * per_day
    ...
    inv = await session.get(Invoice, inv_id)
    if not inv:
        await w.add_balance(user.id, cost, reason="refund:renew_no_invoice", actor_id=user.id)
        await cb.message.answer("Invoice not found")
        return
    try:
...
async def extra_volume(cb: CallbackQuery, session, user):
    ...
    per_gb = int((await session.execute(select(BotSetting.value).where(BotSetting.key == "Extra_volume"))).scalar_one_or_none() or 0) or 2000
    gb = 5
    cost = gb * per_gb
    w = WalletService(session)
    ...
    try:
        ...
        await api.update_user(inv.service_username, volume_gb=(inv.volume_gb or 0) + gb)
...
async def _panel_for(session, name):
    ...
    row = (await session.execute(select(PanelServer).where(PanelServer.name == name))).scalar_one_or_none()
    if not row:
        raise PurchaseError(f"panel {name} not found")
    cls = registry.get("panel", row.plugin, row.revision)
    if not cls:
        raise PurchaseError(f"plugin {row.plugin}/{row.revision} not registered")
    cfg = {"url": row.url, "username": row.username, "password": row.password, "sub_url_base": row.sub_url_base, "inbound_id": row.inbound_id}
    cfg.update(row.extra or {})
    inst = cls(config=cfg)
...
async def accept_rules(cb: CallbackQuery, session, user):
    ...
    await cb.message.answer(t("users.start", name=cb.from_user.first_name or "", shop="Mirza"), reply_markup=main_menu_kb())
